...
# Launch test cluster
#
if ENV['SERVER'] and not Stretchysearch::Extensions::Test::Cluster.running?
...
#
at_exit { Stretchysearch::Extensions::Test::Cluster.stop if ENV['SERVER'] and Stretchysearch::Extensions::Test::Cluster.running? }
...
$client ||= Stretchysearch::Client.new url: url
$helper_client ||= Stretchysearch::Client.new url: url

$client.transport.logger = logger unless ENV['QUIET'] || ENV['CI']
# $client.transport.tracer = tracer if ENV['CI']

# Store Stretchysearch version
#
es_version_info = $client.info['version']
$es_version = es_version_info['number']

puts '-'*80,
     "Stretchysearch #{$es_version.ansi(:bold)} [#{es_version_info['build_hash'].to_s[0...7]}]".center(80),
     '-'*80
...
suites.each do |suite|
  name = Stretchysearch::YamlTestSuite::Utils.titleize(suite.basename)

  Stretchysearch::YamlTestSuite::Runner.in_context name do
                  ...
                  begin
                    ...
                    if catch_exception
                      ...
                      if 'param' == catch_exception
                        ...
                        if e.class.to_s =~ /Stretchysearch/
