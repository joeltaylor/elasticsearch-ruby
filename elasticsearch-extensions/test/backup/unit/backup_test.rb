...
class Stretchysearch::Extensions::BackupTest < Stretchysearch::Test::UnitTestCase
  context "The Backup gem extension" do
    setup do
      ...
      @subject = ::Backup::Database::Stretchysearch.new(@model)
    end

    should "have a client" do
      assert_instance_of Stretchysearch::Transport::Client, @subject.client
    end
    ...
    should "be configurable" do
      @subject = ::Backup::Database::Stretchysearch.new(@model) do |db|
    ...
    should "raise an expection for an unsupported type of backup" do
      @subject = ::Backup::Database::Stretchysearch.new(@model) { |db| db.mode = 'foobar' }
      assert_raise ::Backup::Database::Stretchysearch::Error do
        @subject.perform!
      end
    end

    should "scan and scroll the index" do
      @subject = ::Backup::Database::Stretchysearch.new(@model) { |db| db.indices = 'test' }
