...
module Stretchysearch
  module Extensions
    module Test
      ...
      # A convenience Ruby class for starting and stopping an Stretchysearch cluster,
      ...
      module Cluster
        ...
        class Cluster
          ...
          #      Stretchysearch::Extensions::Test::Cluster::Cluster.new.start
          #
          ...
          def start
            if self.running?
              __log "[!] Stretchysearch cluster already running".ansi(:red)
              return false
            end
            ...
            __log "Starting ".ansi(:faint) + arguments[:number_of_nodes].to_s.ansi(:bold, :faint) +
                  " Stretchysearch #{arguments[:number_of_nodes] < 2 ? 'node' : 'nodes'}..".ansi(:faint), :print

            pids = []

            __log "\nUsing Stretchysearch version [#{version}]" if ENV['DEBUG']
          ...
          #      Stretchysearch::Extensions::Test::Cluster::Cluster.new.stop
          #
          # @example Stop the cluster reachable on specific port
          #      Stretchysearch::Extensions::Test::Cluster::Cluster.new(port: 9350).stop
          #
          # @return Boolean,Array
          # @see Cluster#start
          #
          def stop
            ...
            unless pids.empty?
              __log "Stopping Stretchysearch nodes... ".ansi(:faint), :print
          ...
          # Returns the major version of Stretchysearch
          #
          # @return String
          ...
          def __determine_version
            ...
            version = if arguments[:version]
              ...
              __log "[!] Cannot find Stretchysearch .jar from path to command [#{arguments[:command]}], using `#{arguments[:command]} --version`" if ENV['DEBUG']
              ...
              unless File.exist? arguments[:command]
                ...
                if full_path.empty?
                  raise Errno::ENOENT, "Cannot find Stretchysearch launch script from [#{arguments[:command]}] -- did you pass a correct path?"
                end
              end
              ...
              if output.empty?
                raise RuntimeError, "Cannot determine Stretchysearch version from [#{arguments[:command]} --version] or [#{arguments[:command]} -v]"
              end
