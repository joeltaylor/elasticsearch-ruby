...
To connect to a specific Stretchysearch host:

    Stretchysearch::Client.new host: 'search.myserver.com'

To connect to a host with specific port:

    Stretchysearch::Client.new host: 'myhost:8080'

To connect to multiple hosts:

    Stretchysearch::Client.new hosts: ['myhost1', 'myhost2']

Instead of Strings, you can pass host information as an array of Hashes:

    Stretchysearch::Client.new hosts: [ { host: 'myhost1', port: 8080 }, { host: 'myhost2', port: 8080 } ]
...
Common URL parts -- scheme, HTTP authentication credentials, URL prefixes, etc -- are handled automatically:
    ...
    Stretchysearch::Client.new url: 'https://username:password@api.server.org:4430/search'

You can pass multiple URLs separated by a comma:

    Stretchysearch::Client.new urls: 'http://localhost:9200,http://localhost:9201'
...
You can pass the authentication credentials, scheme and port in the host configuration hash:
    ...
    Stretchysearch::Client.new hosts: [
...
... or simply use the common URL format:
    ...
    Stretchysearch::Client.new url: 'https://username:password@example.com:9200'
...
set the `log` argument:
    ...
    Stretchysearch::Client.new log: true

To trace requests and responses in the _Curl_ format, set the `trace` argument:

    Stretchysearch::Client.new trace: true
...
Or, you can use a custom {::Logger} instance:
    ...
    Stretchysearch::Client.new logger: Logger.new(STDERR)
...
You can pass the client any conforming logger implementation:
    ...
    client = Stretchysearch::Client.new logger: log

### Setting Timeouts

For many operations in Stretchysearch, the default timeouts of HTTP libraries are too low.
To increase the timeout, you can use the `request_timeout` parameter:

    Stretchysearch::Client.new request_timeout: 5*60
...
collection on initialization and reloading:
    ...
    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], randomize_hosts: true
...
on a different host:
    ...
    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: true

You can specify how many times should the client retry the request before it raises an exception
(the default is 3 times):

    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], retry_on_failure: 5

### Reloading Hosts

Stretchysearch by default dynamically discovers new nodes in the cluster. You can leverage this
...
You can pass a specific number of requests after which the reloading should be performed:
    ...
    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_connections: 1_000

To reload connections on failures, use:

    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], reload_on_failure: true

The reloading will timeout if not finished under 1 second by default. To change the setting:

    Stretchysearch::Client.new hosts: ['localhost:9200', 'localhost:9201'], sniffer_timeout: 3
...
**NOTE:** When using reloading hosts ("sniffing") together with authentication, just pass the scheme,
    ...
    Stretchysearch::Client.new host: 'localhost:9200',
...
{Stretchysearch::Transport::Transport::Connections::Selector::RoundRobin} strategy.
...
Only when these would be unavailable, the strategy will use the other nodes:
    ...
    class RackIdSelector
      include Stretchysearch::Transport::Transport::Connections::Selector::Base
    ...
    Stretchysearch::Client.new hosts: ['x1.search.org', 'x2.search.org'], selector_class: RackIdSelector
...
To use a specific adapter for _Faraday_, pass it as the `adapter` argument:
    ...
    client = Stretchysearch::Client.new adapter: :net_http_persistent
...
constructor, use the `transport_options` key:
    ...
    client = Stretchysearch::Client.new transport_options: {
...
To configure the _Faraday_ instance directly, use a block:
    ...
    client = Stretchysearch::Client.new(host: 'localhost', port: '9200') do |f|
      f.response :logger
      f.adapter  :typhoeus
    end
...
for example sign the requests for the [AWS Stretchysearch service](https://aws.amazon.com/elasticsearch-service/):
    ...
    client = Stretchysearch::Client.new url: 'https://search-my-cluster-abc123....es.amazonaws.com' do |f|
...
as the `transport` argument:
    ...
    #
    client = Stretchysearch::Client.new transport: transport
...
Instead of passing the transport to the constructor, you can inject it at run time:
    ...
    faraday_client = Stretchysearch::Transport::Transport::HTTP::Faraday.new \
...
{Stretchysearch::Transport::Transport::Base} module, implementing the required contract,
...
{Stretchysearch::Transport::Transport::Serializer::Base} module, implementing the required contract,
...
The highest-level exception is {Stretchysearch::Transport::Transport::Error}
and will be raised for any generic client *or* server errors.

{Stretchysearch::Transport::Transport::ServerError} will be raised for server errors only.

As an example for response-specific errors, a `404` response status will raise
an {Stretchysearch::Transport::Transport::Errors::NotFound} exception.

Finally, {Stretchysearch::Transport::Transport::SnifferTimeoutError} will be raised
...
* {Stretchysearch::Transport::Client} is composed of {Stretchysearch::Transport::Transport}

* {Stretchysearch::Transport::Transport} is composed of {Stretchysearch::Transport::Transport::Connections},
...
* The {Stretchysearch::Transport::Transport::Serializer::Base} implementations handle converting data for Stretchysearch
  (eg. to JSON). You can implement your own serializer.

* {Stretchysearch::Transport::Transport::Sniffer} allows to discover nodes in the cluster and use them as connections.

* {Stretchysearch::Transport::Transport::Connections::Collection} is composed of
  {Stretchysearch::Transport::Transport::Connections::Connection} instances and a selector instance.

* {Stretchysearch::Transport::Transport::Connections::Connection} contains the connection attributes such as hostname and port,
  as well as the concrete persistent "session" connected to a specific node.

* The {Stretchysearch::Transport::Transport::Connections::Selector::Base} implementations allow to choose connections
